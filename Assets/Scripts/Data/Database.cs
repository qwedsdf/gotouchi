...
public class Database : MonoBehaviour
{
    ...
    void checkpath()
    {
        ...
        GameObject.Find("EventSystem").GetComponent<button>().check("うわああ");
        string[] files = System.IO.Directory.GetFiles(@str, "*.png", System.IO.SearchOption.AllDirectories);
        GameObject.Find("EventSystem").GetComponent<button>().check("大きさは" + files.Length);
        for (int i = 0; i < files.Length; i++)
        {
            if (System.IO.Path.GetExtension(files[i]) == IMG_EXTENSION)
            {
                this.GetComponent<button>().check("パスは" + files[i]);
            }
        }
    }
